...
def make_update_packs(loa: list[Asset]) -> list[UpdatePack]:
    """
    1. Скачиваем лист "Анкетирование"
    2. Делаем список для пакетов обновления
    Цикл FOR:
        1. Собираем пустой UpdatePack
        2. Сразу добавляем аттрибут ссылку на анкету, бэкап линк не берем, он будет статический
        3. Собираем full_unit
        Цикл FOR:
            1. Берем full_unit и обходим все ассеты из параметра loa
            2. Если full_unit включен в full_units ассета — добавляем актив в assets
        4. Добавляем заполненный UpdatePack в лист пакетов обновления
    3. Возвращаем лист пакетов обновления
...
def make_survey_backups():
    """
    Тут не важно какие были ассеты, это самостоятельная функция
    1. Собираем лист "анкетирование"
    2. В нем собираем все текущие данные
    3. Копируем их на лист "backup"
    !!! Важно сделать так, чтобы бэкап 100% происходит всего один раз !!!
...
def update_surveys(update_packs: list[UpdatePack]):
    """
    - Цикл FOR:
        1. Берем поочередно апдейт паки
        2. В апдейт-паке формируем ссылку
        3. Загружаем в нее ассеты методами из коллекта
    Та дааааа!
